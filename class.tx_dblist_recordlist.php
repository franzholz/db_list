 ...
 * @see localRecordList
 */
class tx_dblist_recordlist extends ux_t3lib_recordList {
	...
	var $displayFilter;				// Array, filter functions FHO
	...
	var $setFilter=array();				// Filter to display for the current table   FHO
...
// FHO begin
		$this->displayFilter = t3lib_div::_GP('displayFilter');
// FHO end
	...
	function generateList()	{
		...
		// FHO start
				if (isset($this->setFilter[$tableName]) && trim($this->setFilter[$tableName]) != '')	{
					$addWhere = 'AND ' . trim($this->setFilter[$tableName]);
				} else {
					$addWhere = '';
				}
		// FHO end
				...
				$this->HTMLcode.=$this->getTable($tableName, $this->id, implode(',',$fields), $addWhere); // FHO
			}
		}
	}
	...
	function setDispFields()	{
		...
		// FHO start
	/**
	 * Setting the sql filters to display in extended list.
	 * Sets the internal variable $this->setFilter
	 *
	 * @return	void
	 */
	function setDispFilter()	{

			// Getting from session:
		$dispFields = $GLOBALS['BE_USER']->getModuleData('list/displayFields');

			// If fields has been inputted, then set those as the value and push it to session variable:
		if (is_array($this->displayFilter))	{
			reset($this->displayFilter);
			$tKey = key($this->displayFilter);
			if ($dispFilter[$tKey] != $this->displayFilter[$tKey])	{
				$this->firstElementNumber = 0;
			}
			$dispFilter[$tKey]=$this->displayFilter[$tKey];
			$GLOBALS['BE_USER']->pushModuleData('list/displayFilter',$dispFilter);
		}

			// Setting result:
		$this->setFilter=$dispFilter;
	}
		// FHO end
...
if (defined('TYPO3_MODE') && $GLOBALS['TYPO3_CONF_VARS'][TYPO3_MODE]['XCLASS']['ext/db_list/class.tx_dblist_recordlist.php'])	{
	include_once($GLOBALS['TYPO3_CONF_VARS'][TYPO3_MODE]['XCLASS']['ext/db_list/class.tx_dblist_recordlist.php']);
}
