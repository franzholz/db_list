...
class tx_dblist_script {
	...
	function main()	{
		...
		/** @var $dblist tx_dblist_localrecordlist */
		$dblist = t3lib_div::makeInstance('tx_dblist_localrecordlist'); // FHO
	...
	// FHO start
			...
			$dblist->start($this->id,$this->table,$this->pointer,$this->search_field,$this->search_levels,$this->showLimit,$script);
			$dblist->setDispFilter();
	...
	// FHO end
		...
		if ($dblist->HTMLcode)	{
			...
			if ($dblist->table)	{
				$this->body .= $dblist->filterInput($dblist->table);	// FHO
...
if (defined('TYPO3_MODE') && $GLOBALS['TYPO3_CONF_VARS'][TYPO3_MODE]['XCLASS']['ext/db_list/class.tx_dblist_script.php'])	{
	include_once($GLOBALS['TYPO3_CONF_VARS'][TYPO3_MODE]['XCLASS']['ext/db_list/class.tx_dblist_script.php']);
}
